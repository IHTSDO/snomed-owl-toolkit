...
class ClassificationResultsWriterAnnotated {
    ...
    void writeResultsRf2Archive(
            ...
            Map<Long, Set<Description>> conceptDescriptionMap,
        ...
        try {
            try (ZipOutputStream zipOutputStream = new ZipOutputStream(resultsOutputStream, UTF_8_CHARSET);
                ...
                writeRelationshipChanges(writer, changeCollector.getAddedStatements(), changeCollector.getRemovedStatements(), conceptDescriptionMap);
    ...
    private void writeRelationshipChanges(
            ...
            Map<Long, Set<Relationship>> removedStatements,
            Map<Long, Set<Description>> conceptDescriptionMap) throws IOException {

        // Write newly inferred relationships
        for (Long sourceId : addedStatements.keySet()) {
            String active = "1";
            Set<Description> sourceDescriptions = conceptDescriptionMap.get(sourceId);
            String sourceTerm = "";
            if(sourceDescriptions != null) {
                for (Description description : sourceDescriptions) {
                    String typeId = description.getTypeId();
                    if (Concepts.FSN.equals(typeId)) {
                        sourceTerm = description.getTerm();
                    }
                }
            }
            ...
            for (Relationship relationship : addedStatements.get(sourceId)) {
                ...
                Set<Description> destionationDescriptions = conceptDescriptionMap.get(destinationId);
                String destinationTerm = "";
                if(destionationDescriptions != null) {
                    for (Description description : destionationDescriptions) {
                        String typeId = description.getTypeId();
                        if (Concepts.FSN.equals(typeId)) {
                            destinationTerm = description.getTerm();
                        }
                    }
                }
                Long relationshipId = relationship.getRelationshipId();
                Set<Description> relationshipDescriptions = conceptDescriptionMap.get(relationshipId);

                Long relationshipTypeId = relationship.getTypeId();
                Set<Description> relationshipTypeDescriptions = conceptDescriptionMap.get(relationshipTypeId);
        ...
        for (Long sourceId : removedStatements.keySet()) {
            ...
            for (Relationship relationship : removedStatements.get(sourceId)) {
                Long destinationId = relationship.getDestinationId();
                Set<Description> destinationDescriptions = conceptDescriptionMap.get(destinationId);
                String destinationTerm = "";
                if(destinationDescriptions != null) {
                    for (Description description : destinationDescriptions) {
                        String typeId = description.getTypeId();
                        if (Concepts.FSN.equals(typeId)) {
                            destinationTerm = description.getTerm();
                        }
